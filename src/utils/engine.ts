...
import { AnthropicEngine } from '../engine/anthropic.js'
//import { AzureEngine } from '../engine/azure'
import { AiEngine, AiEngineConfig } from '../engine/Engine.js'
import { FlowiseEngine } from '../engine/flowise.js'
import { GeminiEngine } from '../engine/gemini.js'
import { OllamaEngine } from '../engine/ollama.js'
import { OpenAiEngine } from '../engine/openAi.js'
import { MistralAiEngine } from '../engine/mistral.js'
import { TestAi, TestMockType } from '../engine/testAi.js'
import { GroqEngine } from '../engine/groq.js'
import { MLXEngine } from '../engine/mlx.js'
...
export function getEngine(): AiEngine {
  ...
  switch (provider) {
    ...
    case OCO_AI_PROVIDER_ENUM.AZURE:
      DEFAULT_CONFIG.defaultQuery = {
        'api-version': config.OCO_OPENAI_VERSION ?? '2024-10-21',
      }
      DEFAULT_CONFIG.defaultHeaders = { 'api-key': config.OCO_API_KEY }
      if (config.OCO_API_URL) {
        DEFAULT_CONFIG.baseURL =
          config.OCO_API_URL + 'openai/deployments/' + config.OCO_MODEL
      }
      return new OpenAiEngine(DEFAULT_CONFIG)
